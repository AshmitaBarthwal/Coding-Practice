...
bool solved(vector<vector<int>> &mat, int r, int c, vector<vector<bool>> &rows, vector<vector<bool>> &cols, vector<vector<vector<bool>>> &box) {
    if (c == 9) return solved(mat, r + 1, 0, rows, cols, box);
    if (r == 9) return true;
    if (mat[r][c] != 0) return solved(mat, r, c + 1, rows, cols, box);
    
    for (int k = 1; k < 10; k++) {
        if (rows[r][k] || cols[c][k] || box[r / 3][c / 3][k]) continue;
        ...
        if (solved(mat, r, c + 1, rows, cols, box)) return true;
